...
//  			evt.preventDefault();
  		...
  		$(".fileDrop").on("drop", function(evt) {
  			...
  			$.ajax({
  		         ...
  		         contentType : false, // 기본값 (application/x-www-form-urlencoded) <- true 일때가 기본값
  		         success : function(data) { // 통신 성공시 수행될 콜백 함수
					...
					let isImg = false;
  		            ...
  		            if (!isImg) {
  		            	// 이미지 파일이 아니면 
  		            	output += "<a href='../../resources/uploads/lcj" + data + "' >" + data + "</a>";
  		            	$("#upfileName").val(data);
  		            }
	            	
  		            output += "<span id='" + data + "' onclick='delFile(this);'>" + "<img src='../../resources/images/lcj/close.png' width='40px' />" + "</span>";
  	...
  	function delFile(obj) {
		...
		let url = '/board/proud/delFile'
  			$.ajax({
  		         url : url, // ajax와 통신 할 곳
  		         data : {origin : origin, thumb, thumb, notImg : notImg}, // 서블릿에 보낼 데이터
  		         dataType : "text", // 수신될 데이터의 타입
  		         type : "post", // 통신 방식
